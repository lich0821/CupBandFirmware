...
//	nrf_gpio_pin_set(MMA8451_EN);	
	...
	nrf_delay_ms(10);
	transfer_succeeded &= mma8451_register_write(MMA8451_CTRL_REG1, 0x01);
...
xyzdata_t mma8451_get_data(void)
{
	...
	if( mma8451_register_read(MMA8451_OUT_X_MSB, &rawData[0], 6) )
	{
		accelValue.x = (rawData[0] << 6) + (rawData[1] >> 2);
		accelValue.y = (rawData[2] << 6) + (rawData[3] >> 2);
		accelValue.z = (rawData[4] << 6) + (rawData[5] >> 2);
